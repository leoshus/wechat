	...
	$.fn.zyUpload = function(options,param){
		...
		return this.each(function(){
			...
			var defaults = {
					...
					url              : "../../fileService/upload",  	// 上传文件的路径
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
				    ...
				    html += '							<img class="upload_image" src="../../images/upload/add_img.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';
				    html += '						</div>';
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
			...
			this.funFilterEligibleFile = function(files){
				...
				for (var i = 0, file; file = files[i]; i++) {
					if (file.size >= 500*1024) {
						alert('您这个"'+ file.name +'"文件大小过大 。</n>抱歉，小本服务器 文件限制在500k一下!');	
					} else {
						// 在这里需要判断当前所有文件中
						arrFiles.push(file);	
					}
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				// 处理不同类型文件代表的图标
				var fileImgSrc = "../../images/upload/fileType/";
